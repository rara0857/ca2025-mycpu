  ...
  when(io.memory_read_enable) {
    ...
    io.wb_memory_read_data := MuxLookup(io.funct3, 0.U)(
      Seq(
        ...
        InstructionsTypeL.lb  -> Cat(Fill(Parameters.DataBits - Parameters.ByteBits, byte(7)), byte),

        // TODO: Complete LBU (zero-extend byte)
        // Hint: Fill upper bits with zero, then concatenate with byte
        InstructionsTypeL.lbu -> Cat(Fill(Parameters.DataBits - Parameters.ByteBits, 0.U(1.W)), byte),
        ...
        InstructionsTypeL.lhu -> Cat(Fill(Parameters.DataBits - 16, 0.U(1.W)), half),
  ...
  // - SH to address 0x1002 (index=2): data[15:0] → bytes 2-3, strobes[2:3]=1
    ...
    switch(io.funct3) {
      is(InstructionsTypeS.sb) {
        ...
        writeData := Cat(0.U((Parameters.DataBits - Parameters.ByteBits).W), data(7, 0)) << (mem_address_index << 3)
      }
      is(InstructionsTypeS.sh) {
        ...
        when(mem_address_index(1) === 0.U) {
          ...
          writeData := Cat(0.U((Parameters.DataBits - 16).W), data(15, 0))
        }.otherwise {
          ...
          writeData := Cat(0.U((Parameters.DataBits - 16).W), data(15, 0)) << 16
        }
      }
