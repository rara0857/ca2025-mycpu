...
int main(int argc, char **argv)
{
    ...
    for (int i = 1; i < argc; i++) {
        if ((!strcmp(argv[i], "-instruction") || !strcmp(argv[i], "-i")) &&
            ...
            vcd_file = argv[++i];
        else if (!strcmp(argv[i], "-vcd-start") && i + 1 < argc)
            vcd_start = std::atoll(argv[++i]);
        else if (!strcmp(argv[i], "-vcd-cycles") && i + 1 < argc)
            vcd_cycles = std::atoll(argv[++i]);
    ...
    VCDTracer vcd_tracer;
    if (vcd_file) {
        vcd_tracer.enable(vcd_file, *top);
        std::cout << "VCD enabled: " << vcd_file << " (start=" << vcd_start << ", cycles=" << vcd_cycles << ")\n";
    }
    ...
    while (cycle < max_cycles && !Verilated::gotFinish()) {
        ...
        if (vcd_file && cycle >= vcd_start && cycle < vcd_start + vcd_cycles) {
